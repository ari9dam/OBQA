...
from pytorch_pretrained_bert.modeling import BertForMultipleChoice,BertPreTrainedModel,BertModel
from torch.nn import CrossEntropyLoss                                           
...
Input = collections.namedtuple("Input","idx passage a b c label")


class BertForMultipleChoiceV2(BertPreTrainedModel):
    def __init__(self, config, num_choices):
        super(BertForMultipleChoiceV2, self).__init__(config)
        self.num_choices = num_choices
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(1024*2, 3)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        pooled_output = pooled_output.view(-1,1024*2)
...
class BertForMultipleChoiceCNN(BertPreTrainedModel):
    def __init__(self, config, num_choices):
        super(BertForMultipleChoiceCNN, self).__init__(config)
        self.num_choices = num_choices
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier1 = nn.Linear(config.hidden_size, 100)
#         self.classifier2 = nn.Linear(1124, 1)
        self.classifier2 = nn.Linear(1124*2, 3)
        self.relu = torch.nn.ReLU()
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1))
        flat_attention_mask = attention_mask.view(-1, attention_mask.size(-1))
        encodings, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask, output_all_encoded_layers=False)
        ...
        all_features = all_features.view(-1,1124*2)
...
class BertQAV2:
    def __init__(self,output_dir,model="best_model.bin",topk=1,
        ...
        if action == "train":
            ...
            model = BertForMultipleChoiceV2.from_pretrained(bert_model,
        ...
        if action == "predict":
            ...
            model = BertForMultipleChoiceV2.from_pretrained(bert_model, state_dict=model_state_dict, num_choices=num_labels)
            model.to(device)
            model = torch.nn.DataParallel(model)
            self.model = model
...
#             examples.append(InputExample(idx=inp.idx, context_sentence=inp.passage, ending_0=inp.a,ending_1=inp.b,ending_2=inp.c, label=inp.label))
            examples.append(InputExample(idx=inp.idx, context_sentence=inp.passage, ending_0=inp.a,ending_1=inp.b, label=inp.label))
        return examples
    ...
    def score_qa(self,eval_dataloader,eval_index,data,model,typ,ep,method="sum"):      
        ...
        for input_ids, input_mask, segment_ids, label_ids,unique_ids in tq:
            ...
            for q,scores in zip(unique_ids,logits):                             
                ...
                idxsplit = idx.split(":")
                qid = idxsplit[0]
                label_index = idxsplit[-1]
                choiceA_index = int(idxsplit[1])
                choiceB_index = int(idxsplit[2])
                
                if qid not in all_results_sum:
                    all_results_sum[qid]=[0]*4
                    all_results_max[qid]=[0]*4
                    qid_label_map[qid]= label_index
                
                all_results_sum[qid][choiceA_index]+=scores[0]
                all_results_sum[qid][choiceB_index]+=scores[1]
                all_results_max[qid][choiceA_index]=max(all_results_max[qid][choiceA_index],scores[0])
                all_results_max[qid][choiceB_index]=max(all_results_max[qid][choiceB_index],scores[1])
        ...
        for index,(qid,scores) in enumerate(all_results_sum.items()):
            maxidx = np.argsort(scores).tolist()[-1]
            if int(qid_label_map[qid]) == maxidx:
                count+=1
#             if index < 5:
#                 print(qid,scores,maxidx,qid_label_map[qid])
