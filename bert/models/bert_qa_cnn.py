...
class BertQACNN:
    def __init__(self,output_dir,model="best_model.bin",topk=1,
        ...
        if action == "train":
            ...
            model = BertForMultipleChoiceCNN.from_pretrained(bert_model,
        ...
        if action == "predict":
            ...
            model = BertForMultipleChoiceCNN.from_pretrained(bert_model, state_dict=model_state_dict, num_choices=num_labels)
            model.to(device)
            model = torch.nn.DataParallel(model)
            self.model = model
