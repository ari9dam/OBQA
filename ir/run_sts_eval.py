...
def main():
    ...
    parser.add_argument('--model',
                        type=str, default="pytorch_model.bin.1",
                        help="pytorch_model.bin.1\n")
    ...
    output_model_file = os.path.join(args.output_dir, args.model)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tq = tqdm(eval_dataloader, desc="Evaluating")
        from torch.nn import MSELoss                                            
        loss_fct = MSELoss() 
        for input_ids, input_mask, segment_ids, label_ids in tq:
            ...
            logits = logits.detach().cpu().numpy()
            label_ids = label_ids.to('cpu').numpy()
            tmp_eval_accuracy = pearson(logits, label_ids)
            tmp_eval_accuracy = 1.0 if math.isnan(tmp_eval_accuracy) else tmp_eval_accuracy
            ...
            tq.set_description("Loss:"+str(eval_loss/nb_eval_steps)+",Pearson:"+str(eval_accuracy/nb_eval_steps))
