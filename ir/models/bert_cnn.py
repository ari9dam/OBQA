...
class BertCNNIR:
    def __init__(self,output_dir,model,topk=50,
                 ...
                 eval_batch_size=64,max_seq_length=128,num_labels=2,entail_label=1):
        print("Loading BERT CNNIR Model")
        self.name = "BertCNNIR"
        self.topk=topk
        ...
        model = BertForCNN.from_pretrained(bert_model, state_dict=model_state_dict, num_labels=num_labels)
        model.half()
        model.to(device)
    ...
    def create_eval_examples(self,hyps,facts):
        examples = []
    ...
    def get_features(self,eval_examples,max_seq_length,tokenizer,tokenfile,chunk_id):
    ...
    def predict(self,data,outfile,tokenfile,no_cache=False):
        hyps = data["data"]
        facts = data["facts"]
        ...
        eval_examples = self.create_eval_examples(hyps,facts)
        logger.info("***** Running evaluation *****")
        logger.info("  Num examples = %d", len(eval_examples))
        logger.info("  Batch size = %d", eval_batch_size)
        all_results = {} 
        chunk_id = 0
        for chunk in chunks(eval_examples,60000):        
            eval_features, evalindex = self.get_features(chunk,max_seq_length,tokenizer,tokenfile,chunk_id)       
            chunk_id+=1
