...
from metrics.overlap_metric import OverlapMetric
...
@Model.register("copynetObqa")
class OBQACopyNetSeq2Seq(Model):
    """
    ...
    def __init__(self,
                 ...
                 overlap_based_metric: Metric = OverlapMetric(),
        ...
        self._overlap_based_metric = overlap_based_metric
    ...
    def forward(self,  # type: ignore
        ...
        if not self.training:
            ...
            if target_tokens:
                ...
                if self._overlap_based_metric is not None:
                    predicted_tokens = self._get_predicted_tokens(output_dict["predictions"],
                                                                  metadata,     
                                                                  n_best=3)
                    self._overlap_based_metric(predicted_tokens,  
                                             [x["target_tokens"] for x in metadata])
    ...
    def _get_predicted_tokens(self,
        ...
        for top_k_predictions, metadata in zip(predicted_indices, batch_metadata):
            ...
            if n_best == 1:
                ...
                predicted_tokens.append(batch_predicted_tokens[0:3])
            else:
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if not self.training:
            ...
            if self._overlap_based_metric is not None:
                all_metrics.update(self._overlap_based_metric.get_metric(reset=reset)) # type: ignore
        return all_metrics
